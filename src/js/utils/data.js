...
function updatePlayers(players,scorers_data) {
	 //var players = {};

		scorers_data.forEach(function(scorer){
		
			players.scorers.filter(function(d){				
				return d.id == scorer.key;
			})
			.forEach(function(d){
				d.goals=scorer.goalsData;
				d.dateStart=scorer.dateStart;
				d.dateEnd=scorer.dateEnd;
				d.goals=scorer.goalsData;
				//d.dates=scorer.values.dates;	
			})
